...
$exampleDigits = [7,4,9,5,11,17,23,2,0,14,21,24,10,16,13,6,15,25,12,22,18,20,8,19,3,26,1];
$exampleBoards = [[
    [22, 13, 17, 11,  0,],
    [ 8,  2, 23,  4, 24,],
    [21,  9, 14, 16,  7,],
...
function playSquidBingo(array $boards, array $numbers): ?int {
    $markedBoards = getEmptyBoards($boards);

    $notWinBoardsCount = count($boards);
    foreach ($numbers as $number) {
        $winnerBoardIndexList = playBingoWithSeveralBoards($boards, $markedBoards, $number);
        if ($winnerBoardIndexList !== null) {
            $notWinBoardsCount -= count($winnerBoardIndexList);

            if ($notWinBoardsCount === 0) {
            ...
            foreach ($winnerBoardIndexList as $winnerBoardIndex) {
                unset($boards[$winnerBoardIndex], $markedBoards[$winnerBoardIndex]);
            }
        }
    }
...
$score = playSquidBingo($exampleBoards, $exampleDigits);
echo sprintf("example squid bingo score is: %d\n", $score);

$score = playSquidBingo($boards, $digits);
echo sprintf("squid bingo score is: %d\n", $score);
